...
=head2 exists CHROM[, START, END]

Test if the chromosome position exist in the reference sequence.
...
sub get_base
{
    ...
    if ($arg->{noMask}) {
        return uc(substr($$self{chunk},$idx,1));     
    }
    else {
...
sub get_slice
{
    ...
    if($arg->{noMask}) {
        return uc(substr($$self{chunk},$from-$$self{from},$to-$from+1));   
    }
    else {
